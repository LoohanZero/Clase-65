...
const obtenerImagenes = (pagina = 0, breed) => {
  axios
    .get(
      `https://api.thecatapi.com/v1/images/search?breed_ids=${breed}&limit=9&page=${pagina}&order=desc&api_key=6a093eb3-b481-44c9-b306-bf1486088068`
    )
    .then((response) => {
...
const actualizarRazas = (razas) => {
  ...
  dropdown.forEach((dropdown) => { dropdown.innerHTML = htmlRazas })
...
const inicializarRazas = () => {
  ...
  $('#breed-dropdown').addEventListener('change', (evento) => {
    ...
    $('#breed-search-img-dropdown').addEventListener('change', (evento) => {
        obtenerImagenes(0 , evento.target.value)
    })
...
const inicializarBusquedaImagenes = () => {
  ...
  $('#next-page-btn').addEventListener('click', () => {
    const breed = $('#breed-search-img-dropdown').value

    pagina++
    obtenerImagenes(pagina, breed)
